 ...
 */
var currentBranchTask = function (lastTaskResult, next) {
    var repo = lastTaskResult['repo'];
    repo.getCurrentBranch()
        .then(
            function (reference) {
                //get refs name
                var localBranch = reference.name();
                lastTaskResult['currentBranch'] = localBranch;
                next(null, lastTaskResult);
            },
            function (err) {
                next(err);
            }
 ...
 * @return lastTaskResult
 */
var referenceListTask = function (lastTaskResult, next) {
    var repo = lastTaskResult['repo'];
    repo.getReferences(Reference.TYPE.LISTALL)
        .then(
            function (refs) {
                lastTaskResult['refs'] = refs;
                next(null, lastTaskResult);
            },
            function (err) {
                next(err);
